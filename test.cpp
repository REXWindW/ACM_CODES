	...
	while(1){
		...
		if(dianti.zhuangtai==0){//停止状态下 
			...
			if(v.size()==0){
				...
				dianti.to=vv[0].second;//找到最小的to 
				if(dianti.to<dianti.lc()){
					dianti.zhuangtai=-1;
				}else dianti.zhuangtai=1;
			}
        }
        else if(dianti.zhuangtai==1){//上行状态中 
            if((dianti.mi-1)%4==0){
                dianti.tichu(dianti.lc());
                q[dianti.lc()].quchu();
                vec=q[dianti.lc()].get();
                for(int j=0;j<n;j++){ 
                    if(vec[j].to>vec[j].from&&dianti.zl+vec[j].weight<=dianti.hezai){//顺便接要往上的人 
                        dianti.ren.push(vec[j]);
                        q[dianti.lc()].tichu(vec[j].num);
                        MAX(dianti.to,vec[j].to);//更新要到的最高楼层 
                    }
                }
                if(dianti.to==dianti.lc()){
                    dianti.zhuangtai=0;
