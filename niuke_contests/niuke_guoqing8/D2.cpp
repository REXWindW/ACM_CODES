...
const int MAXN = 37;
int dp[MAXN][MAXN][MAXN][MAXN];//四维背包
                ...
                repb(ic,0,C){
                    repb(im,0,M){
                        if(ip>=p[i]&&ia>=a[i]&&ic>=c[i]&&im>=m[i]){//可以放得下
                            if(dp[ip][ia][ic][im]<dp[ip-p[i]][ia-a[i]][ic-c[i]][im-m[i]]+g[i]){
                                dp[ip][ia][ic][im]=dp[ip-p[i]][ia-a[i]][ic-c[i]][im-m[i]]+g[i];
                                path[i][ip][ia][ic][im]=1;
                            } 
                            else dp[ip][ia][ic][im]=dp[ip][ia][ic][im];
                        }
                    }
                }
            }
