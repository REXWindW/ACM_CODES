...
void dfs(int now,int prev){
	...
	for(auto x:e[now]){
		if(x.first==prev) continue;
		res[now] = (res[now]+ ( ((x.second*cnt1[x.first]+sum1[x.first])%Mod)*(cnt0[now]-cnt0[x.first]+(color[now]-1)) )%Mod )%Mod;
		res[now] = (res[now]+ ( ((x.second*cnt0[x.first]+sum0[x.first])%Mod)*(cnt1[now]-cnt1[x.first]-color[now]) )%Mod )%Mod;
		//cout<<"res between "<<x.first<<"with others == "<<res[now]<<endl;
		//子树之间形成的关系
		if(color[now]==1){
