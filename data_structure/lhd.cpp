...
void dfs(int now,int dep){
    if(!flg[now]){
        ...
        cout<<"↳"<<tree[now].v.num<<' '<<tree[now].v.name<<endl;
        if(tree[now].r){
            if(tree[now].l)
                v.push_back(1);
            else v.push_back(0);
...
void search(){
    ...
    for(int i=1;i<=tree_tail;i++){
        int flag=0;
        int val=calc(tmp,tree[i].v.name);
        srt[++cnt]=make_pair(val,i);
    }
    sort(srt+1,srt+cnt+1);
    cout<<"为您找到以下字段:"<<endl;
    for(int i=1;i<=min(10,cnt);i++){
        cout<<tree[srt[i].second].v.num<<" "<<tree[srt[i].second].v.name<<endl;
    }
    system("pause");
}
inline void del(){
